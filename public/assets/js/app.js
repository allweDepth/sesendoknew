	...
	loadModule(url) {
		...
		if (moduleName === "tata_naskah") {
			...
			if (!this.loadedModules.includes("document_builder")) {
				...
				builderScript.src = window.appUrl("/assets/js/engine/document/document_builder.js?v=20260905.3"); // cache-bust plain input + metadata-only
		...
		script.src = window.appUrl(`/assets/js/modules/${moduleName}.js`) + `?v=${encodeURIComponent(window.ASSET_VERSION||Date.now())}.20260905.8`; // cache-bust setiap rilis
