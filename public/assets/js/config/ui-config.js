...
UIConfig.sub_kegiatan_renstra_neo = {
	...
	form: {
		elements: [
			...
			{ tag: "fieldTextarea", prop: { label: "Indikator Keluaran", name: "indikator_keluaran", atribut: `rows="2"`, width: 16 } },
			{ tag: "field", prop: { label: "Satuan", name: "satuan", width: 16 } },
...
UIConfig.trx_naskah_dinas = {
	...
	form: {
		elements: [
			...
			{ tag: "fieldCalendar", prop: { label: "Tanggal Dibuat", name: "tgl_insert", calendarType: "datetime" } },
		],
	},
};
UIConfig.rekanan = {
...
UIConfig.absensi={validation:{pegawai_id:{required:true},tanggal:{required:true},status:{required:true}},form:{elements:[
	...
	{tag:"fieldCalendar",prop:{label:"Jam Masuk",name:"jam_masuk",calendarType:"time"}},{tag:"fieldCalendar",prop:{label:"Jam Pulang",name:"jam_pulang",calendarType:"time"}},
...
UIConfig.penugasan_subkegiatan={form:{elements:[
	...
	{tag:"fieldDropdown",prop:{label:"Peran",name:"peran",options:["KEPALA_OPD","PA_KPA","PPK","PPTK","PPK_SKPD","BENDAHARA","PEJABAT_PENGADAAN","POKJA_ULP","STAF","VIEWER"].map(x=>({value:x,text:x.replaceAll('_',' ')}))}},
...
const sakipInput=(label,name,type="text")=>({tag:"input",prop:{label,name,type}}),sakipText=(label,name)=>({tag:"fieldTextarea",prop:{label,name,atribut:'rows="3"'}}),sakipSelect=(label,name,options)=>({tag:"fieldDropdown",prop:{label,name,options:options.map(x=>({value:x,text:x.replaceAll("_"," ").toUpperCase()}))}}),sakipLookup=(label,name,source)=>({tag:"fieldDropdown",prop:{label,name,source,search:true}});
UIConfig.iku_opd={form:{elements:[sakipLookup("Renstra","renstra_id","renstra_dropdown"),sakipLookup("Sasaran Strategis","sasaran_renstra_id","sasaran_renstra_dropdown"),sakipLookup("Program Renstra","program_renstra_id","program_renstra_dropdown"),sakipInput("Kode IKU","kode_iku"),sakipText("Nama Indikator","nama_indikator"),sakipText("Definisi Operasional","definisi_operasional"),sakipText("Formula Perhitungan","formula_perhitungan"),sakipInput("Satuan","satuan"),sakipSelect("Polaritas","polaritas",["maksimal","minimal","stabil"]),sakipInput("Sumber Data","sumber_data"),sakipSelect("Frekuensi","frekuensi_pengukuran",["bulanan","triwulanan","semesteran","tahunan"]),sakipLookup("Penanggung Jawab Struktur Aktif (otomatis bila kosong)","penanggung_jawab_struktur_id","struktur_jabatan_aktif_dropdown"),... ["baseline","target_t1","target_t2","target_t3","target_t4","target_t5","target_akhir"].map(x=>sakipInput(x.replaceAll("_"," ").toUpperCase(),x,"number")),sakipSelect("Status","status",["draft","ditetapkan","direviu"]),sakipInput("Nomor Penetapan","nomor_penetapan"),{tag:"fieldCalendar",prop:{label:"Tanggal Penetapan",name:"tanggal_penetapan",calendarType:"date"}},sakipText("Keterangan","keterangan")]}};
UIConfig.pohon_kinerja={form:{elements:[sakipLookup("Induk Kinerja","parent_id","pohon_kinerja_dropdown"),sakipLookup("Renstra","renstra_id","renstra_dropdown"),sakipInput("Tahun","tahun","number"),sakipInput("Kode Kinerja","kode_kinerja"),sakipText("Uraian Kinerja","uraian_kinerja"),sakipSelect("Jenjang","jenjang",["strategis","taktis","operasional"]),sakipSelect("Jenis Kinerja","jenis_kinerja",["outcome","intermediate_outcome","output"]),sakipLookup("Sumber Dokumen Renstra","sumber_kinerja_key","sakip_sumber_dropdown"),sakipLookup("IKU Terkait","iku_id","iku_dropdown"),sakipText("Indikator","indikator"),sakipInput("Satuan","satuan"),sakipInput("Target","target","number"),sakipLookup("Penanggung Jawab Struktur Aktif (otomatis bila kosong)","penanggung_jawab_struktur_id","struktur_jabatan_aktif_dropdown"),sakipText("Hubungan Sebab Akibat","hubungan_sebab_akibat"),sakipSelect("Status","status",["draft","ditetapkan","direviu"]),sakipText("Keterangan","keterangan")]}};
UIConfig.perjanjian_kinerja={form:{elements:[sakipInput("Nomor Dokumen","nomor_dokumen"),{tag:"fieldCalendar",prop:{label:"Tanggal Dokumen",name:"tanggal_dokumen",calendarType:"date"}},sakipSelect("Jenis","jenis",["awal","perubahan"]),sakipLookup("Pihak Pertama dari Struktur (otomatis bila kosong)","pihak_pertama_struktur_id","struktur_jabatan_aktif_dropdown"),sakipInput("Dasar Dokumen","dasar_dokumen"),sakipSelect("Status","status",["draft","diajukan","ditetapkan","direviu"]),{tag:"fieldCalendar",prop:{label:"Tanggal Penetapan",name:"tanggal_penetapan",calendarType:"date"}},sakipText("Keterangan","keterangan")]}};
UIConfig.perjanjian_kinerja_detail={form:{elements:[sakipLookup("Perjanjian Kinerja","perjanjian_kinerja_id","pk_dropdown"),sakipInput("Nomor Urut","nomor_urut","number"),sakipLookup("Pohon Kinerja","pohon_kinerja_id","pohon_kinerja_dropdown"),sakipLookup("IKU","iku_id","iku_dropdown"),sakipText("Sasaran Kinerja","sasaran_kinerja"),sakipText("Indikator Kinerja","indikator_kinerja"),sakipInput("Satuan","satuan"),sakipInput("Target","target","number"),sakipText("Program/Kegiatan","program_kegiatan"),sakipLookup("Sub Kegiatan","kd_sub_keg","sub_kegiatan_penugasan"),sakipInput("Anggaran","anggaran","number"),sakipSelect("Sumber Anggaran","sumber_anggaran",["renja","rka","dpa","manual"]),sakipText("Keterangan","keterangan")]}};
UIConfig.pengukuran_kinerja={form:{elements:[sakipLookup("Indikator PK","perjanjian_kinerja_detail_id","pk_detail_dropdown"),sakipSelect("Periode","periode",["bulanan","triwulanan","semesteran","tahunan"]),sakipInput("Nomor Periode","nomor_periode","number"),sakipInput("Target Periode","target_periode","number"),sakipInput("Realisasi Periode","realisasi_periode","number"),sakipInput("Realisasi Kumulatif","realisasi_kumulatif","number"),sakipInput("Capaian (%)","capaian_persen","number"),sakipInput("Bukti Dukung/URL","bukti_dukung"),sakipText("Analisis Capaian","analisis_capaian"),sakipText("Kendala","kendala"),sakipText("Tindak Lanjut","tindak_lanjut"),sakipSelect("Status","status",["draft","dikirim","diverifikasi","perbaikan"])]}};
UIConfig.evaluasi_renstra={form:{elements:[sakipLookup("Renstra","renstra_id","renstra_dropdown"),sakipInput("Tahun Evaluasi","tahun_evaluasi","number"),sakipSelect("Jenis Evaluasi","jenis_evaluasi",["tahunan","tengah_periode","akhir_periode","reviu"]),sakipLookup("IKU","iku_id","iku_dropdown"),sakipText("Indikator","indikator"),sakipInput("Satuan","satuan"),sakipInput("Target Tahunan","target_tahunan","number"),sakipInput("Target Kumulatif","target_kumulatif","number"),sakipInput("Realisasi Tahunan","realisasi_tahunan","number"),sakipInput("Realisasi Kumulatif","realisasi_kumulatif","number"),sakipInput("Capaian (%)","capaian_persen","number"),sakipInput("Pagu Anggaran","pagu_anggaran","number"),sakipInput("Realisasi Anggaran","realisasi_anggaran","number"),sakipText("Faktor Pendorong","faktor_pendorong"),sakipText("Faktor Penghambat","faktor_penghambat"),sakipText("Tindak Lanjut","tindak_lanjut"),sakipText("Rekomendasi Reviu","rekomendasi_reviu"),sakipSelect("Status","status",["draft","dikirim","diverifikasi","perbaikan","final"])]}};
UIConfig.renja_kinerja={form:{elements:[sakipLookup("Sumber Sub Kegiatan Renstra","sub_kegiatan_renstra_id","sub_kegiatan_renstra_dropdown"),sakipLookup("Sub Kegiatan Referensi","kd_sub_keg","sub_kegiatan_penugasan"),sakipLookup("Pohon Kinerja","pohon_kinerja_id","pohon_kinerja_dropdown"),sakipText("Indikator Keluaran","indikator_keluaran"),sakipInput("Satuan","satuan"),sakipInput("Target","target","number"),sakipInput("Lokasi","lokasi"),sakipInput("Kelompok Sasaran","kelompok_sasaran"),sakipInput("Pagu Indikatif","pagu_indikatif","number"),sakipInput("Prakiraan Maju Target","prakiraan_maju_target","number"),sakipInput("Prakiraan Maju Pagu","prakiraan_maju_pagu","number"),sakipInput("Sumber Dana","sumber_dana"),sakipText("Catatan Penting","catatan_penting"),sakipSelect("Status","status",["draft","diajukan","disetujui"])]}};
[
