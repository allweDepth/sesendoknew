...
class KontrakRealisasiService
{
    ...
    {
        [$w,$o,$y]=$this->scope();$scope='k.kd_wilayah=? AND k.tahun=? AND k.is_deleted=0';$params=[$w,$y];if($o&&$o!=='0'){$scope.=' AND k.kd_opd=?';$params[]=$o;}
        ...
        $grouped=[];foreach($budget as $row){$key=$row['kd_sub_keg'].'|'.$row['kd_akun'];$grouped[$key]=['kd_sub_keg'=>$row['kd_sub_keg'],'kd_akun'=>$row['kd_akun'],'pagu'=>(float)$row['pagu'],'realisasi'=>0.0];}foreach($actual as $row){$key=$row['kd_sub_keg'].'|'.$row['kd_akun'];$grouped[$key]??=['kd_sub_keg'=>$row['kd_sub_keg'],'kd_akun'=>$row['kd_akun'],'pagu'=>0.0,'realisasi'=>0.0];$grouped[$key]['realisasi']=(float)$row['realisasi'];}foreach($grouped as &$row){$row['sisa']=$row['pagu']-$row['realisasi'];$row['persen']=$row['pagu']>0?$row['realisasi']/$row['pagu']*100:0;}unset($row);return array_values($grouped);
    }

    public function monthlyFinancialRows(): array
    {
        $details=$this->financialRows();$months=[];for($month=1;$month<=12;$month++)$months[$month]=['bulan'=>$month,'realisasi'=>0.0,'fisik_total'=>0.0,'keuangan_total'=>0.0,'transaksi'=>0];foreach($details as $row){$month=(int)($row['periode']?:date('n',strtotime($row['tanggal'])));if($month<1||$month>12)continue;$months[$month]['realisasi']+=(float)$row['jumlah'];$months[$month]['fisik_total']+=(float)$row['progress_fisik'];$months[$month]['keuangan_total']+=(float)$row['progress_keuangan'];$months[$month]['transaksi']++;}foreach($months as &$row){$count=max(1,$row['transaksi']);$row['fisik']=$row['fisik_total']/$count;$row['keuangan']=$row['keuangan_total']/$count;unset($row['fisik_total'],$row['keuangan_total']);}unset($row);return array_values($months);
    }

    public function financialExcel(string $format): string
    {
        $format=in_array($format,['spj','lra','bulanan_fisik_keuangan'],true)?$format:'lra';[,,$year]=$this->scope();$book=new Spreadsheet();$s=$book->getActiveSheet();$titles=['spj'=>'SPJ Bendahara','lra'=>'Laporan Realisasi Anggaran','bulanan_fisik_keuangan'=>'Bulanan Fisik Keuangan'];$s->setTitle($titles[$format]);
        $headers=[];$dataRows=[];$moneyColumns=[];$percentColumns=[];
        if($format==='spj'){$headers=['NO','TANGGAL','NOMOR BUKTI','NOMOR KONTRAK','PENYEDIA','URAIAN','KODE REKENING','JUMLAH'];foreach($this->financialRows() as $i=>$x)$dataRows[]=[$i+1,$x['tanggal'],$x['nomor_bukti'],$x['nomor_kontrak'],$x['penyedia'],$x['uraian'],$x['kd_akun'],(float)$x['jumlah']];$moneyColumns=['H'];}
        elseif($format==='lra'){$headers=['NO','SUB KEGIATAN','KODE REKENING','PAGU ANGGARAN','REALISASI','SISA ANGGARAN','CAPAIAN %'];foreach($this->lraRows() as $i=>$x)$dataRows[]=[$i+1,$x['kd_sub_keg'],$x['kd_akun'],$x['pagu'],$x['realisasi'],$x['sisa'],$x['persen']];$moneyColumns=['D','E','F'];$percentColumns=['G'];}
        else{$headers=['NO','BULAN','JUMLAH TRANSAKSI','REALISASI KEUANGAN','PROGRES FISIK %','PROGRES KEUANGAN %'];$monthNames=[1=>'Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'];foreach($this->monthlyFinancialRows() as $i=>$x)$dataRows[]=[$i+1,$monthNames[$x['bulan']],$x['transaksi'],$x['realisasi'],$x['fisik'],$x['keuangan']];$moneyColumns=['D'];$percentColumns=['E','F'];}
        ...
        $s->fromArray($headers,null,'A4');$row=5;
        foreach($dataRows as $values)$s->fromArray($values,null,'A'.$row++);
        $totalRow=$row;$s->setCellValue('A'.$totalRow,'TOTAL');$s->mergeCells('A'.$totalRow.':C'.$totalRow);
        if($format==='spj')$s->setCellValue('H'.$totalRow,'=SUM(H5:H'.($row-1).')');
        elseif($format==='lra'){foreach(['D','E','F'] as $column)$s->setCellValue($column.$totalRow,'=SUM('.$column.'5:'.$column.($row-1).')');$s->setCellValue('G'.$totalRow,"=IF(D{$totalRow}=0,0,E{$totalRow}/D{$totalRow}*100)");}
        else{$s->setCellValue('D'.$totalRow,'=SUM(D5:D'.($row-1).')');}
        $s->getStyle("A1:{$last}1")->getFont()->setBold(true)->setSize(14)->getColor()->setARGB('FFFFFFFF');$s->getStyle("A1:{$last}1")->getFill()->setFillType('solid')->getStartColor()->setARGB('FF174A68');$s->getStyle("A4:{$last}4")->getFont()->setBold(true)->getColor()->setARGB('FFFFFFFF');$s->getStyle("A4:{$last}4")->getFill()->setFillType('solid')->getStartColor()->setARGB('FF2185D0');$s->getStyle("A4:{$last}{$totalRow}")->getBorders()->getAllBorders()->setBorderStyle(Border::BORDER_THIN);$s->getStyle("A{$totalRow}:{$last}{$totalRow}")->getFont()->setBold(true);foreach($moneyColumns as $column)$s->getStyle("{$column}5:{$column}{$totalRow}")->getNumberFormat()->setFormatCode('#,##0.00');foreach($percentColumns as $column)$s->getStyle("{$column}5:{$column}{$totalRow}")->getNumberFormat()->setFormatCode('0.00"%"');foreach(range('A',$last) as $column)$s->getColumnDimension($column)->setAutoSize(true);$s->freezePane('A5');$s->setAutoFilter("A4:{$last}".max(4,$row-1));
        if($format!=='spj'&&$dataRows){$count=count($dataRows);$categoryColumn=$format==='lra'?'C':'B';$valueColumns=$format==='lra'?['D','E']:['E','F'];$labels=[];$values=[];foreach($valueColumns as $column){$labels[]=new DataSeriesValues(DataSeriesValues::DATASERIES_TYPE_STRING,"'{$s->getTitle()}'!\${$column}\$4",null,1);$values[]=new DataSeriesValues(DataSeriesValues::DATASERIES_TYPE_NUMBER,"'{$s->getTitle()}'!\${$column}\$5:\${$column}\$".(4+$count),null,$count);}$categories=[new DataSeriesValues(DataSeriesValues::DATASERIES_TYPE_STRING,"'{$s->getTitle()}'!\${$categoryColumn}\$5:\${$categoryColumn}\$".(4+$count),null,$count)];$series=new DataSeries($format==='lra'?DataSeries::TYPE_BARCHART:DataSeries::TYPE_LINECHART,DataSeries::GROUPING_CLUSTERED,range(0,count($values)-1),$labels,$categories,$values);$chart=new Chart('financial_chart',new Title($format==='lra'?'Pagu vs Realisasi':'Progres Fisik dan Keuangan'),new Legend(Legend::POSITION_BOTTOM),new PlotArea(null,[$series]));$chart->setTopLeftPosition($format==='lra'?'I4':'H4');$chart->setBottomRightPosition($format==='lra'?'Q20':'P20');$s->addChart($chart);}
        $tmp=tempnam(sys_get_temp_dir(),'finance_').'.xlsx';$writer=new Xlsx($book);$writer->setIncludeCharts(true);$writer->save($tmp);return $tmp;
    }

    public function financialPdf(string $format): string
    {
        $format=in_array($format,['spj','lra','bulanan_fisik_keuangan'],true)?$format:'lra';[,,$year]=$this->scope();$title=['spj'=>'FORMAT SPJ BENDAHARA','lra'=>'LAPORAN REALISASI ANGGARAN','bulanan_fisik_keuangan'=>'LAPORAN BULANAN FISIK DAN KEUANGAN'][$format];$pdf=new TCPDF('L','mm','A4',true,'UTF-8');$pdf->SetMargins(8,10,8);$pdf->AddPage();$pdf->SetFont('helvetica','B',13);$pdf->Cell(0,8,$title.' TAHUN '.$year,0,1,'C');$pdf->SetFont('helvetica','',7);$e=static fn($v)=>htmlspecialchars((string)$v,ENT_QUOTES,'UTF-8');
        if($format==='spj'){$rows=$this->financialRows();$html='<table border="1" cellpadding="3"><tr style="font-weight:bold;background-color:#dcecff"><th>No</th><th>Tanggal</th><th>Bukti</th><th>Kontrak</th><th>Penyedia</th><th>Uraian</th><th>Rekening</th><th>Jumlah</th></tr>';$total=0;foreach($rows as $i=>$x){$total+=(float)$x['jumlah'];$html.='<tr><td>'.($i+1).'</td><td>'.$e($x['tanggal']).'</td><td>'.$e($x['nomor_bukti']).'</td><td>'.$e($x['nomor_kontrak']).'</td><td>'.$e($x['penyedia']).'</td><td>'.$e($x['uraian']).'</td><td>'.$e($x['kd_akun']).'</td><td align="right">'.number_format((float)$x['jumlah'],0,',','.').'</td></tr>';}$html.='<tr style="font-weight:bold"><td colspan="7" align="right">TOTAL</td><td align="right">'.number_format($total,0,',','.').'</td></tr></table>';}
        elseif($format==='lra'){$rows=$this->lraRows();$html='<table border="1" cellpadding="3"><tr style="font-weight:bold;background-color:#dcecff"><th>No</th><th>Sub Kegiatan</th><th>Rekening</th><th>Pagu</th><th>Realisasi</th><th>Sisa</th><th>Capaian</th></tr>';$pagu=$actual=0;foreach($rows as $i=>$x){$pagu+=$x['pagu'];$actual+=$x['realisasi'];$html.='<tr><td>'.($i+1).'</td><td>'.$e($x['kd_sub_keg']).'</td><td>'.$e($x['kd_akun']).'</td><td align="right">'.number_format($x['pagu'],0,',','.').'</td><td align="right">'.number_format($x['realisasi'],0,',','.').'</td><td align="right">'.number_format($x['sisa'],0,',','.').'</td><td align="right">'.number_format($x['persen'],2,',','.').'%</td></tr>';}$html.='<tr style="font-weight:bold"><td colspan="3" align="right">TOTAL</td><td align="right">'.number_format($pagu,0,',','.').'</td><td align="right">'.number_format($actual,0,',','.').'</td><td align="right">'.number_format($pagu-$actual,0,',','.').'</td><td align="right">'.number_format($pagu>0?$actual/$pagu*100:0,2,',','.').'%</td></tr></table>';}
        else{$rows=$this->monthlyFinancialRows();$names=[1=>'Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'];$html='<table border="1" cellpadding="3"><tr style="font-weight:bold;background-color:#dcecff"><th>No</th><th>Bulan</th><th>Transaksi</th><th>Realisasi</th><th>Fisik</th><th>Keuangan</th></tr>';$max=1;foreach($rows as $x)$max=max($max,$x['fisik'],$x['keuangan']);foreach($rows as $i=>$x)$html.='<tr><td>'.($i+1).'</td><td>'.$names[$x['bulan']].'</td><td>'.$x['transaksi'].'</td><td align="right">'.number_format($x['realisasi'],0,',','.').'</td><td align="right">'.number_format($x['fisik'],2,',','.').'%</td><td align="right">'.number_format($x['keuangan'],2,',','.').'%</td></tr>';$html.='</table>';$pdf->writeHTML($html,true,false,true,false,'');$baseY=$pdf->GetY()+32;$startX=22;$pdf->SetFont('helvetica','',6);foreach($rows as $i=>$x){$xPos=$startX+$i*20;$fh=$x['fisik']/$max*25;$kh=$x['keuangan']/$max*25;$pdf->SetFillColor(33,133,208);$pdf->Rect($xPos,$baseY-$fh,6,$fh,'F');$pdf->SetFillColor(242,113,28);$pdf->Rect($xPos+7,$baseY-$kh,6,$kh,'F');$pdf->Text($xPos,$baseY+1,(string)$x['bulan']);}$pdf->Text(22,$baseY+7,'Biru: fisik | Oranye: keuangan');return $pdf->Output('','S');}
        $pdf->writeHTML($html,true,false,true,false,'');return $pdf->Output('','S');
    }
}
