...
class PdfTemplateService
{
  public function renderOfficial(TCPDF $pdf,array $header,array $schema,array $data,?string $logo=null):void
  {
    ...
    $pdf->SetXY(50,15);
    $pdf->MultiCell(137,6,strtoupper($_SESSION['user']['nama_pemda']??'PEMERINTAH DAERAH'),0,'C');
    $pdf->SetFont('times','B',15);
    $pdf->SetX(50);$pdf->MultiCell(137,7,strtoupper($_SESSION['user']['nama_opd']??'PERANGKAT DAERAH'),0,'C');
    $pdf->SetFont('times','',9);
    $pdf->SetX(50);$pdf->MultiCell(137,5,$_SESSION['user']['alamat_opd']??'',0,'C');
