...
<div class="ui container" id="strukturOrganisasi" data-manage="<?= $canManage ? '1' : '0' ?>">
  <div class="ui blue icon message"><i class="sitemap icon"></i>
    <div class="content">
      <div class="header">Struktur Jabatan Struktural OPD</div>
      <p>Susun jabatan dari Kepala OPD, Sekretaris/Kepala Bidang (Eselon III), hingga jabatan struktural terbawah. Pejabat hanya dapat dipilih dari Data ASN aktif OPD dan tahun berjalan.</p>
    </div>
  </div>
...
<div class="ui modal" id="strukturModal"><i class="close icon"></i>
  ...
  <div class="content">
    <form class="ui form" id="strukturForm"><input type="hidden" name="id">
      ...
      <div class="field"><label>Atasan Langsung</label><select class="ui fluid search dropdown" name="parent_id">
...
<script>
  (() => {
    ...
    function fill() {
      ...
      par.html('<option value="">Tidak ada (pimpinan tertinggi)</option>' + data.rows.map(x => `<option value="${x.id}">${esc(x.nama_jabatan)} — ${esc(x.nama_pegawai||'-')}</option>`).join(''));
    ...
    function render() {
      ...
      const byParent = {};
      data.rows.forEach(x => (byParent[x.parent_id || 0] ??= []).push(x));
      const branch = (pid, depth = 0) => (byParent[pid] || []).map(x => `<div class="ui segment" style="margin-left:${Math.min(depth,5)*28}px;border-left:4px solid ${depth?'#21ba45':'#2185d0'}"><div class="ui right floated buttons">${canManage?`<button class="ui mini basic blue icon button edit-struktur" data-id="${x.id}"><i class="edit icon"></i></button><button class="ui mini basic red icon button hapus-struktur" data-id="${x.id}"><i class="trash icon"></i></button>`:''}</div><div class="ui ${depth?'teal':'blue'} label">${esc(x.eselon||'Non-eselon')}</div> <b>${esc(x.nama_jabatan)}</b><div class="ui small header" style="margin:8px 0 2px">${esc(x.nama_pegawai||'Belum ditetapkan')}</div><div class="meta">NIP ${esc(x.nip||'-')} · ${esc(x.kelompok_jabatan||'Kelompok belum diisi')}</div></div>${branch(x.id,depth+1)}`).join('');
      $('#strukturList').html(branch(0) + ((byParent[0] || []).length ? '' : '<div class="ui warning message">Semua jabatan memiliki atasan. Tetapkan satu jabatan sebagai pimpinan tertinggi.</div>'));
    }
    $(document).off('.struktur').on('change.struktur', '#strukturForm [name="nama_jabatan_preset"]', function() {
