...
<div class="ui container form-wrapper">
    ...
    <div class="ui bottom attached tab segment" data-tab="page-setup">
        <form class="ui form" id="form-page-setup">
            ...
            <div class="six fields"><div class="field"><label>Atas</label><input type="number" min="0" step="0.5" name="margin_atas_mm"></div><div class="field"><label>Kanan</label><input type="number" min="0" step="0.5" name="margin_kanan_mm"></div><div class="field"><label>Bawah</label><input type="number" min="0" step="0.5" name="margin_bawah_mm"></div><div class="field"><label>Kiri</label><input type="number" min="0" step="0.5" name="margin_kiri_mm"></div><div class="field"><label>Jarak header dari tepi</label><input type="number" min="0" step="0.5" name="margin_header_mm"></div><div class="field"><label>Jarak footer dari tepi</label><input type="number" min="0" step="0.5" name="margin_footer_mm"></div></div>
            <div class="ui two stackable cards page-setup-sections">
                <?php foreach(['header'=>'Header','footer'=>'Footer'] as $section=>$sectionLabel):?>
                <div class="fluid card page-section-editor" data-page-section="<?= $section ?>">
                    <div class="content"><div class="right floated ui toggle checkbox"><input type="checkbox" name="<?= $section ?>_pdf_aktif" value="1"><label>Aktif</label></div><div class="header"><i class="<?= $section==='header'?'arrow up':'arrow down' ?> icon"></i><?= $sectionLabel ?> PDF</div><div class="meta">Tiga kolom dengan lebar dan alignment terpisah</div></div>
                    <div class="content">
                        <input type="hidden" name="<?= $section ?>_pdf_json">
                        <div class="four fields"><div class="field"><label>Font</label><select class="ui dropdown" data-section-style="font"><option value="helvetica">Helvetica</option><option value="times">Times</option><option value="courier">Courier</option></select></div><div class="field"><label>Ukuran</label><input type="number" min="5" max="24" step="0.5" data-section-style="size" value="8"></div><div class="field"><label>Warna</label><input type="color" data-section-style="color" value="#222222"></div><div class="field"><label>Tinggi ruang (mm)</label><input type="number" min="4" max="80" step="0.5" name="tinggi_<?= $section ?>_mm" value="<?= $section==='header'?'12':'10' ?>"></div></div>
                        <div class="ui mini basic buttons page-style-buttons"><button type="button" class="ui button" data-section-flag="bold"><i class="bold icon"></i>Tebal</button><button type="button" class="ui button" data-section-flag="italic"><i class="italic icon"></i>Miring</button><button type="button" class="ui button" data-section-flag="underline"><i class="underline icon"></i>Garis bawah</button><button type="button" class="ui button" data-section-flag="divider"><i class="minus icon"></i>Garis pemisah</button></div>
                        <div class="ui divider"></div>
                        <?php foreach([['Kiri',25,'L'],['Tengah',50,'C'],['Kanan',25,'R']] as $index=>$column):?>
                        <div class="fields page-section-column" data-column="<?= $index ?>"><div class="eight wide field"><label>Teks <?= $column[0] ?></label><input type="text" maxlength="500" data-column-key="text" placeholder="<?= $section==='footer'&&$index===2?'{page} / {pages}':'' ?>"></div><div class="four wide field"><label>Lebar (%)</label><input type="number" min="5" max="90" step="0.5" data-column-key="width" value="<?= $column[1] ?>"></div><div class="four wide field"><label>Alignment</label><select class="ui dropdown" data-column-key="align"><option value="L" <?= $column[2]==='L'?'selected':'' ?>>Kiri</option><option value="C" <?= $column[2]==='C'?'selected':'' ?>>Tengah</option><option value="R" <?= $column[2]==='R'?'selected':'' ?>>Kanan</option></select></div></div>
                        <?php endforeach;?>
                        <div class="ui tiny message">Placeholder: <code>{page}</code>, <code>{pages}</code>, <code>{date}</code>, <code>{year}</code>, <code>{opd}</code>, <code>{wilayah}</code>.</div>
                    </div>
                </div>
                <?php endforeach;?>
            </div>
