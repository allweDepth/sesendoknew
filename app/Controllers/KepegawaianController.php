...
class KepegawaianController extends Controller
{
  ...
  public function structureSave(): void
  {
    ...
    try {
      ...
      if ($id) {
        ...
        $db->update('struktur_jabatan_opd_neo', $data, 'WHERE id=? AND kd_wilayah=? AND kd_opd=? AND tahun=? AND is_deleted=0', [$id, $u['kd_wilayah'], $u['kd_opd'], $u['tahun']]);
      } else {
        $data += ['kd_wilayah' => $u['kd_wilayah'], 'kd_opd' => $u['kd_opd'], 'tahun' => $u['tahun'], 'tgl_insert' => date('Y-m-d H:i:s'), 'username_insert' => $u['username'] ?? 'system', 'is_deleted' => 0];
  ...
  public function structureDelete(): void
  {
    ...
    try {
      ...
      $db->begin();
      try {
        $db->query('UPDATE struktur_jabatan_opd_neo SET parent_id=NULL,tgl_update=NOW(),username_update=? WHERE parent_id=? AND kd_wilayah=? AND kd_opd=? AND tahun=? AND is_deleted=0', [$u['username'] ?? 'system', $id, $u['kd_wilayah'], $u['kd_opd'], $u['tahun']]);
        $db->query('UPDATE struktur_jabatan_opd_neo SET is_deleted=1,tgl_update=NOW(),username_update=? WHERE id=? AND kd_wilayah=? AND kd_opd=? AND tahun=?', [$u['username'] ?? 'system', $id, $u['kd_wilayah'], $u['kd_opd'], $u['tahun']]);
        $db->commit();
      } catch (Throwable $e) {
        $db->rollback();
        throw $e;
