...
class AnggaranController extends Controller
{
    ...
    public function approval():void
    {
        ...
        if($_SERVER['REQUEST_METHOD']!=='POST'){echo JsonResponse::error('Method tidak diizinkan',405);return;}
        if(empty($_SESSION['csrf_token'])||($_SERVER['HTTP_X_CSRF_TOKEN']??'')!==$_SESSION['csrf_token']){echo JsonResponse::error('CSRF validation gagal',403);return;}
        try{$data=(new AnggaranDocumentService(Auth::scopedUser()))->setApproval((string)($_POST['tbl']??''),(string)($_POST['kd_sub_keg']??''),!empty($_POST['approved']),(string)($_POST['scenario']??'KUNCI'));echo JsonResponse::success(!empty($_POST['approved'])?'Dokumen disetujui dan skenario TAPD selesai diproses':'Persetujuan dokumen dibuka kembali',[],$data);}catch(Throwable $e){echo JsonResponse::error($e->getMessage(),400);}
    }
    ...
    public function exportSamePriceExcel():void{try{if(!Auth::check())throw new RuntimeException('Sesi login tidak valid');$logical=(string)($_GET['tbl']??'');$file=(new AnggaranDocumentService($_SESSION['user']??[]))->exportSamePriceExcel($logical);header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');header('Content-Disposition: attachment; filename="'.$logical.'-rekap-harga-satuan-sama.xlsx"');readfile($file);unlink($file);}catch(Throwable $e){http_response_code(400);echo$e->getMessage();}exit;}
