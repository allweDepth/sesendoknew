...
class AnggaranController extends Controller
{
    ...
    public function tapdSave():void{header('Content-Type: application/json;charset=UTF-8');try{if(!Auth::check())throw new RuntimeException('Sesi login tidak valid');$role=$_SESSION['user']['type_user']??'';if(!in_array($role,['super_admin','admin_wilayah','tapd'],true))throw new RuntimeException('Hanya administrator daerah/TAPD yang dapat mengatur penugasan');$start=(string)($_POST['tanggal_mulai']??'');$end=(string)($_POST['tanggal_selesai']??'');if(!$start||!$end||$start>$end||trim((string)($_POST['nama']??''))==='')throw new InvalidArgumentException('Nama dan masa berlaku penugasan wajib valid');DB::getInstance()->insert('tapd_penugasan_neo',['kd_wilayah'=>$_SESSION['user']['kd_wilayah'],'tahun'=>$_SESSION['user']['tahun'],'user_id'=>$_POST['user_id']?:null,'nama'=>trim($_POST['nama']),'nip'=>trim($_POST['nip']??''),'jabatan'=>trim($_POST['jabatan']??'Anggota'),'urutan'=>(int)($_POST['urutan']??1),'tanggal_mulai'=>$start,'tanggal_selesai'=>$end,'aktif'=>1,'username_insert'=>$_SESSION['user']['username']??'system','is_deleted'=>0]);echo JsonResponse::success('Penugasan TAPD tersimpan');}catch(Throwable $e){echo JsonResponse::error($e->getMessage(),400);}}
}
